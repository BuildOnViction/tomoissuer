...
contract BridgeToken is Ownable, Pausable, ReentrancyGuard {
    ...
    function withdrawEth(address payable recipient, uint256 amount, uint256 txId, string calldata txHash, bytes calldata signature) external nonReentrant whenNotPaused transactionExists(txHash) {
    ...
    function withdrawERC20(IERC20 token, address recipient, uint256 amount, uint256 txId, string calldata txHash, bytes calldata signature) external whenNotPaused nonReentrant transactionExists(txHash) {
    ...
    function ownerWithdrawTokens(IERC20 token, uint amount) external onlyOwner {
